...
# Суть в том что  обращаемся к аргументу id в url_for() разными способами в зависимости от контекста.
# В первом случае, переменная url_id, а во втором как атрибут id объекта url
# Во втором случае, когда мы добавляем новый URL и получаем объект url из базы данных,
# мы используем атрибут id этого объекта, и id в этом случае является ссылкой на идентификатор объекта url
# Поэтому затруднительно будет сократить, тот вариант который ты мне предложил,
# вбрасывает ошибку связанную с этим id или url_id :(
